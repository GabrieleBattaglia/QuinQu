...
#QConstants
VERSIONE="1.10.3 del 3 aprile 2024"
...
def VPTempo(valori,datainizio,datafine,obiettivo):
	...
	print(f"Oggi è il {Humanize(oggi)}, giorno {(oggi-datainizio).days} di {durata.days}. Sei al {(oggi1-d1)*100/(d2-d1):+.2f}%, in frazione: {fractions.Fraction((oggi-datainizio).days, durata.days)}\n\tdel periodo di tempo impostato.")
	return

def VPObiettivo(valori, datainizio,datafine,obiettivo):
	...
	print(f"Valore iniziale {valoreiniziale:+.2f}, attuale {valoreattuale:+.2f}, obiettivo {obiettivo:+.2f} = ({obiettivo-valoreiniziale:+.2f}).")
	print(f"Ottenuto {valoreattuale-valoreiniziale:+.2f} pari al {(valoreattuale-valoreiniziale)*100/(obiettivo-valoreiniziale):+.2f}%, in frazione: {fractions.Fraction(int(valoreattuale-valoreiniziale), int(obiettivo-valoreiniziale))}")
	print(f"Da fare {obiettivo-valoreattuale:+.2f} pari al {(obiettivo-valoreattuale)*100/(obiettivo-valoreiniziale):+.2f}%")
	return
...
def Inizializzazione():
	'''Inizializza e restituisce i 4 dati fondamentali'''
	...
	return valori,datainizio,datafine,obiettivo

def Reset(valori,datainizio,datafine,obiettivo):
	'''Inizializza tutti i dati'''
	attesa=dgt(prompt="ATTENZIONE! Sei sicuro di voler cancellare tutto?\n\tL'operazione è irreversibile! Digita 'sicuro'> ",kind="s",smin=6,smax=6,default="n")
	if attesa == "sicuro":
		valori,datainizio,datafine,obiettivo=Inizializzazione()
...
try:
	...
	VPObiettivo(valori,datainizio,datafine,obiettivo)
except IOError:
	valori,datainizio,datafine,obiettivo=Inizializzazione()
	Salva()
	print("Iniziamo")
...
while True:
	...
	elif attesa=="b": VPObiettivo(valori,datainizio,datafine,obiettivo)
	...
	elif attesa=="q": valori,datainizio,datafine,obiettivo=Reset(valori,datainizio,datafine,obiettivo)
	elif attesa=="t": VPTempo(valori,datainizio,datafine,obiettivo)
	elif attesa=="o":
